    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mt-10 pt-6 border-t border-gray-100">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="https://wa.me/?text={{ urlencode($berita->BERITA_JUDUL . ' - ' . url()->current()) }}"
                           ...
                           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#25D366] text-white text-sm font-medium hover:bg-[#1ebe5d] transition-colors">
                        ...
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
                           target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#1877F2] text-white text-sm font-medium hover:bg-[#1464d3] transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Twitter/X --}}
                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($berita->BERITA_JUDUL) }}"
                           target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-black text-white text-sm font-medium hover:bg-gray-800 transition-colors">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($beritaTerkait as $terkait)
                    <article class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col">
                        <a href="{{ route('berita.show', $terkait->BERITA_SLUG) }}" class="block overflow-hidden">
                            <img src="{{ $terkait->thumbnail_url }}"
                                 alt="{{ $terkait->BERITA_JUDUL }}"
                                 class="w-full h-40 object-cover hover:scale-105 transition-transform duration-300">
                        </a>
                        <div class="p-4 flex flex-col flex-1">
                            <span class="inline-block text-xs font-semibold px-2.5 py-1 rounded-full mb-2 {{ $terkait->badge_color }} w-fit">
                                {{ $terkait->BERITA_KATEGORI }}
                            </span>
                            <a href="{{ route('berita.show', $terkait->BERITA_SLUG) }}"
                               class="text-gray-800 font-semibold text-sm leading-snug hover:text-green-600 transition-colors line-clamp-2 flex-1">
                                {{ $terkait->BERITA_JUDUL }}
                            </a>
                            <p class="text-xs text-gray-400 mt-2">{{ $terkait->tanggal_publish }}</p>
                        </div>
                    </article>
                @endforeach
            </div>
        ...
        <div class="mt-8 text-center">
            <a href="{{ route('berita.index') }}"
               class="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-green-600 text-green-600 text-sm font-semibold hover:bg-green-600 hover:text-white transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
