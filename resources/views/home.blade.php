...
<section id="berita" class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 class="text-3xl md:text-4xl font-bold text-center mb-12 text-gray-800">Berita & Pengumuman</h2>
         
        {{-- Grid Berita --}}
        @if($beritaTerbaru->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @include('berita._card', ['beritaList' => $beritaTerbaru])
            </div>

            <!-- Lihat Semua -->
            <div class="flex justify-end mt-6">
                <a href="{{ route('berita.index') }}" 
                   class="inline-flex items-center gap-2 text-lg font-medium text-blue-600 hover:text-blue-800 transition-colors">
                    Lihat semua
